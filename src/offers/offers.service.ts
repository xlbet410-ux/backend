...
export class OffersService implements OnModuleInit, OnModuleDestroy {
  ...
  private toAdmin(
    ...
    return {
      ...
      imageUrlEn: offer.imageUrlEn,
      bannerUrl: offer.bannerUrl,
      bannerUrlEn: offer.bannerUrlEn,
  ...
  async duplicateOffer(id: string) {
    ...
    const offer = await this.prisma.offer.create({
      data: {
        ...
        imageUrlEn: existing.imageUrlEn,
        bannerUrl: existing.bannerUrl,
        bannerUrlEn: existing.bannerUrlEn,
  ...
  // Calendar-month start (UTC, matching every timestamptz column — see
  // startOfUTCMonth), not a rolling 30 days. A user who loses less than the
  // threshold this month simply doesn't reach it; the shortfall never
