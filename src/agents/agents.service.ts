...
export class AgentsService {
  ...
  async recordLossCommission(
    ...
    try {
      ...
      // Agents only earn commission on losses of the player's own principal
      // money, not money the house handed out as a bonus. deposit_turnover
      // doesn't count as "a bonus" here — it's still the player's own
      // deposit, just not fully wagered once yet. Any OTHER active wallet
      // means this bet is being staked with bonus balance, so skip it.
   ...
   * (principal-only — see recordLossCommission). Batched aggregates (one
   * groupBy per metric) instead of one query per player.
   *
  ...
  async getReferredPlayerStats(
    ...
    const [deposits, withdrawals, bets, commissions] = await Promise.all([
      ...
      // principal (it skips bets made while a real bonus is active), so
      // this sum is exactly "principal loss," never inflated by offer/bonus
      // wagering. See recordLossCommission for where that split happens.
