  ...
  private async buildCatalog(): Promise<CatalogGame[]> {
    ...
    for (const provider of active) {
      try {
        ...
        // 9Wicket publishes no per-title catalog at all, and Oracle's
        // getProviderGames endpoint isn't reliable for it (it isn't a normal
        // provider) — calling it first and checking isNineWicket only after
        // meant a failed call there dropped 9Wicket from the whole catalog
        // via the catch below. Skip the call entirely and synthesize the one
        // card it needs directly, so it flows through the exact same
        // pipeline (category, overrides, isActive, thumbnail) as every real
        // game without depending on that endpoint at all.
        const games = isNineWicket
          ? [
              {
                name: '9Wicket',
          ...
          : ((
              (await this.getProviderGames(provider.code)) as {
                games?: Array<{
                  name: string;
                  game_uid: string;
                  category: string;
